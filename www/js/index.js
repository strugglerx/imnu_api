    ...
    methods:{
        open: function() {
            ...
            if (user.length != 11||code.length !=4){
                ...
                document.getElementById("preloader").style.display="block";
                axios.get("http://localhost:8081/api/score",{params: {user: user, pwd: pwd, vrcode: code, session: window.session}})
        ...
        login: function() {
            ...
            if (user.length != 11){
                ...
                document.getElementById("preloader").style.display="block";
                axios.get("http://localhost:8081/api/st",{params: {user: user, pwd: pwd}})
                    .then(response => {console.log(response);
                                        let res = response.data.datas.err;
                                        //console.log(res);
                                        if (!res) {
